...
import {
  JobOptions, ReferOptions, TaskOptions, Questions,
} from '../../constants';
...
const Job = (props) => {
  ...
  const {
    user, isAuthed, token, refreshToken,
  ...
  useEffect(() => {
    ...
    const taskOptions = TaskOptions.map((taskOpt) => ({ checked: false, ...taskOpt }));
    setTaskOptions(taskOptions);

    const referOptions = ReferOptions.map((refereOpt) => ({ checked: false, ...refereOpt }));
    setReferOptions(referOptions);
  }, [history.location.state, user]);

  const submitQuestions = (e) => {
    e.preventDefault();
    if (!refers) { return setErrorText('Answer required'); }
            ...
            <ul className="m-0">
              <li className="">
                { user && user.misc && user.misc.questionnaire
                  ...
                  : (
                    <a
                      className={`opt ${option && option === JobOptions.questionare
                        ? 'active'
                        : ''}`}
              ...
              <li className="">
                <a
                  className={`opt ${option && option === JobOptions.profile
                    ? 'active'
                    : ''}`}
              ...
              <li className="">
                <a
                  className={`opt ${option && option === JobOptions.referral
                    ? 'active'
                    : ''}`}
